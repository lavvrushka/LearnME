...
{
    internal static class GroupsService
    {
        private static List<Group> groups = new()
    {
        ...
        public static List<Group> GetFeaturedGroups()
        {
            var rnd = new Random();
            var randomizedGroups = groups.OrderBy(item => rnd.Next());

            return randomizedGroups.Take(2).ToList();
        }
    }
}
